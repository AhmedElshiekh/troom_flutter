...
class CourseDetailsCont extends GetxController{
  ...
  List<LessonsList> getLessonList(index){
    ...
    List<LessonsList> lessonList = List<LessonsList>.from(l.map((model)=> LessonsList.fromJson(model))).toList();
    return lessonList;
  }
  ...
  Future<bool> getCourseDetailsData(courseKey,token)async{
    ...
    update();
    var res = await _repo.courseDetails(courseKey, token,appLang.toString()).then((value) {
      CourseDetailsRes courseDetailsRes = CourseDetailsRes.fromJson(value.data);
      if (courseDetailsRes.status){
        courseDetailsResData = CourseDetailsResData.fromJson(courseDetailsRes.data.toJson());
        courseDetailsResData.chapters.forEach((element) {
          myExpandedItem.add(myItem(header: element.name,body: element.lessonsList));
        });
        update();
        print('$LOGD getCourseDetailsData  Name ${jsonEncode(courseDetailsResData)}');
        return true;
      }else{
        return false;
      }
  ...
  Future<bool> getLessonData(currentLessonKey,token)async{
    ...
    var res = await _repo.lessonData(currentLessonKey, token,appLang.toString()).then((value) {
      LessonDataRes lessonDataRes = LessonDataRes.fromJson(value.data);
      if (lessonDataRes.status){
        lessonDataResData = LessonDataResData.fromJson(lessonDataRes.data.toJson());
        update();
        prepareVideo(lessonDataResData.video);
        update();
        print('$LOGD getLessonData  summary ${lessonDataResData.summary}');
        // print('$LOGD Before Check CurrentLessonKey = ${getStorage!.read('CurrentLessonKey')} <--> newLessonKey = ${lessonDataResData.key}');
        if(lessonDataResData.lessonClosed){
          Get.snackbar('', '',backgroundColor: ConstStyles.DarkColor,
              colorText: ConstStyles.WhiteColor,
              titleText: CustomText(txt: 'YouShouldPassTheLessonExamFirst'.tr,
      ...
      modalHudController.changeisLoading(false);
      // if(courseDetailsResData.owner!=null && courseDetailsResData.placementTest != null){
      //   showDialog(
      ...
      //         return AlertDialog(
      //             content:
      //             PlacementTest(localH: null,placementKey: courseDetailsResData.placementTest,),
      //         );
      //         //   GetBuilder<HomeCont>(builder: (_){
      //         //   //TODO Dialog
      //         //   return AlertDialog(
      //         //     content: PlacementTest(localH: null,placementKey: courseDetailsResData.placementTest,),
      //         //   );
      //         // },);
      //       });
      //   return Container();
  ...
  Future<void> getCourseQuizLesson(lessonKey,token)async{
    ...
    var res = await _repo.courseQuizLesson(lessonKey, token).then((value) {
      ...
      if (quizLessonRes.status){
        ...
        questionsLessonList = quizLessonResData.map((e) => QuizLessonQuestionsList(data: e)).toList();
        update();
        print('$LOGD getCourseQuizLesson resData:: ${jsonEncode(quizLessonResData)}');
  ...
  Future<void> getCourseQuizChapter(chapterKey,token,chapterName)async{
    ...
    var res = await _repo.courseQuizChapter(chapterKey, token).then((value) {
      ...
      if (quizLessonRes.status){
        ...
        questionChapterList = quizLessonResData.map((e) => QuizChapterQuestionList(data: e)).toList();
        update();
        print('$LOGD getCourseQuizLesson resData:: ${jsonEncode(quizLessonResData)}');
        print('$LOGD questionChapterList length:: ${questionChapterList.length}');
        // print('$LOGD getCourseQuizLesson Q1:: ${questionsList[0].question}');
      }
    }).whenComplete(() {
      //TODO if questionList > 0 then open exam view
  ...
  submitAnswers(lessonKey){
    ...
    if(checkAnswers.length >= questionsLessonList.length){
      if(checkAnswers.contains(false)){
        quizResultReq = QuizResultReq(lesson_key: lessonKey,result: 0.toString());
        print('$LOGD SubmitAnswer Req False=== ${jsonEncode(quizResultReq)}');
        sendQuizResult(quizResultReq);
      }else{
        quizResultReq = QuizResultReq(lesson_key: lessonKey,result: 1.toString());
  ...
  submitChapterAnswers(chapterKey){
    ...
    if(checkAnswers.length >= questionChapterList.length){
      if(checkAnswers.contains(false)){
        quizChapterResultReq = QuizChapterResultReq(chapter_key: chapterKey,result: 0.toString());
        print('$LOGD submitChapterAnswers Req False=== ${jsonEncode(quizChapterResultReq)}');
        // Get.snackbar('', '',backgroundColor: ConstStyles.DarkColor,
        //     colorText: ConstStyles.WhiteColor,
        ...
        quizChapterResultReq = QuizChapterResultReq(chapter_key: chapterKey,result: 1.toString());
        print('$LOGD submitChapterAnswers Req True=== ${jsonEncode(quizChapterResultReq)}');
        // Get.snackbar('', '',backgroundColor: ConstStyles.DarkColor,
        //     colorText: ConstStyles.WhiteColor,
  ...
  sendQuizResult(QuizResultReq quizReq)async{
    ...
    var res = await _repo.quizResult(quizReq,token).then((value) {
      ...
      if(quizResultRes.status){
        quizResultResData = QuizResultResData.fromJson(quizResultRes.data.toJson());
        ...
        if(quizResultResData.pass == '0'){
          Get.snackbar('', '',backgroundColor: ConstStyles.DarkColor,
              colorText: ConstStyles.WhiteColor,
              titleText: CustomText(txt: 'FailedToPassLessonExam'.tr,
                txtAlign: TextAlign.center,));

        }else{
          Get.snackbar('', '',backgroundColor: ConstStyles.DarkColor,
              colorText: ConstStyles.WhiteColor,
              titleText: CustomText(txt: 'SuccessfullyPassExam'.tr,
                txtAlign: TextAlign.center,));
        }
        update();
...
*/
  ...
  // String path2 = await ExtStorage.getExternalStoragePublicDirectory(ExtStorage.DIRECTORY_DOWNLOADS);
    ...
    try{
      ...
      File file = File("$path/${lessonDataResData.name}.pdf");
  ...
  Future<bool> getPlacementExamData(examKey,token,appLang)async{
    ...
    var res = await homeRepo.getPlacementExam(examKey,token,appLang.toString()).then((value) {
  ...
  submitPlacementAnswers(req,token,appLang)async{
    ...
    update();
    HomeRepo homeRepo = HomeRepo();
    var res = await homeRepo.submitPlacementAnswers(req,token,appLang).then((value) {
